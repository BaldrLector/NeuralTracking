...
if __name__ == "__main__":
    ...
    scene_flow_path = os.path.join(
        seq_dir, "scene_flow", "shirt_000000_000110.sflow")
    ...
    point_image = image_proc.backproject_depth(
        depth_image, fx, fy, cx, cy, normalizer=DEPTH_NORMALIZER)
    ...
    assert np.isfinite(node_deformations).all(
    ), "All deformations should be valid."
    ...
    utils.save_grayscale_image("example_data/output/pixel_anchors_mask.jpeg", pixel_anchors_mask)
    ...
    if not os.path.exists(dst_graph_nodes_dir):
        os.makedirs(dst_graph_nodes_dir)
    ...
    dst_node_deformations_dir = os.path.join(
        seq_dir, "graph_node_deformations")
    ...
    output_graph_nodes_path = os.path.join(
        dst_graph_nodes_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", NODE_COVERAGE))
    output_graph_edges_path = os.path.join(
        dst_graph_edges_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", NODE_COVERAGE))
    output_graph_edges_weights_path = os.path.join(
        dst_graph_edges_weights_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", NODE_COVERAGE))
    output_node_deformations_path = os.path.join(
        dst_node_deformations_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", NODE_COVERAGE))
    output_graph_clusters_path = os.path.join(
        dst_graph_clusters_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", NODE_COVERAGE))
    output_pixel_anchors_path = os.path.join(
        dst_pixel_anchors_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", NODE_COVERAGE))
    output_pixel_weights_path = os.path.join(
        dst_pixel_weights_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", NODE_COVERAGE))

    utils.save_graph_nodes(output_graph_nodes_path, node_coords)
    utils.save_graph_edges(output_graph_edges_path, graph_edges)
    utils.save_graph_edges_weights(
        output_graph_edges_weights_path, graph_edges_weights)
    utils.save_graph_node_deformations(
        output_node_deformations_path, node_deformations)
    ...
    assert np.array_equal(
        node_coords, utils.load_graph_nodes(output_graph_nodes_path))
    assert np.array_equal(
        graph_edges, utils.load_graph_edges(output_graph_edges_path))
    assert np.array_equal(graph_edges_weights, utils.load_graph_edges_weights(
        output_graph_edges_weights_path))
    assert np.allclose(node_deformations, utils.load_graph_node_deformations(
        output_node_deformations_path))
    assert np.array_equal(
        graph_clusters, utils.load_graph_clusters(output_graph_clusters_path))
    assert np.array_equal(
        pixel_anchors, utils.load_int_image(output_pixel_anchors_path))
    assert np.array_equal(
        pixel_weights, utils.load_float_image(output_pixel_weights_path))
