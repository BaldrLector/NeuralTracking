...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_dir', action='store', dest='train_dir',
                        default=r'/media/baldr/新加卷/deepdeform_v1_1/train',
                        help='Provide a subfolder with training data')
    parser.add_argument('--val_dir', action='store', dest='val_dir',
                        default=r'/media/baldr/新加卷/deepdeform_v1_1/val',
                        help='Provide a subfolder with SPARSE validation data')
    parser.add_argument('--experiment', action='store',
                        default=r'debug_flownet',
                        dest='experiment', help='Provide an experiment name')
    parser.add_argument('--date', action='store', dest='date',
                        help='Provide a date in the format %Y-%m-%d (if you do not want current date)')
    ...
    train_log_dir = log_dir + "/" + os.path.basename(train_dir)
    val_log_dir = log_dir + "/" + os.path.basename(val_dir)
    ...
    options_file_in = os.path.abspath(os.path.join(
        os.path.dirname(__file__), "options.py"))
    ...
    if not os.path.exists(saving_model_dir):
        os.makedirs(saving_model_dir,exist_ok=True)
    ...
    if opt.use_pretrained_model:
        ...
        if "chairs_things" in saved_model:
            ...
            if opt.model_module_to_load == "full_model":
                ...
                pretrained_dict = {
                    k: v for k, v in pretrained_dict.items() if "flow_net" in k}
    ...
    # Criterion.
    criterion = DeformLoss(opt.lambda_flow, opt.lambda_graph,
                           opt.lambda_warp, opt.lambda_mask, opt.flow_loss_type)

    # Count parameters.
    n_all_model_params = int(sum([np.prod(p.size())
                                  for p in model.parameters()]))
    n_trainable_model_params = int(sum([np.prod(p.size()) for p in filter(
        lambda p: p.requires_grad, model.parameters())]))
    print(
        "Number of parameters: {0} / {1}".format(n_trainable_model_params, n_all_model_params))

    n_all_flownet_params = int(
        sum([np.prod(p.size()) for p in model.flow_net.parameters()]))
    n_trainable_flownet_params = int(sum([np.prod(p.size()) for p in filter(
        lambda p: p.requires_grad, model.flow_net.parameters())]))
    print(
        "-> Flow network: {0} / {1}".format(n_trainable_flownet_params, n_all_flownet_params))

    n_all_masknet_params = int(
        sum([np.prod(p.size()) for p in model.mask_net.parameters()]))
    n_trainable_masknet_params = int(sum([np.prod(p.size()) for p in filter(
        lambda p: p.requires_grad, model.mask_net.parameters())]))
    print(
        "-> Mask network: {0} / {1}".format(n_trainable_masknet_params, n_all_masknet_params))
    print()

    # Set up optimizer.
    if opt.use_adam:
        optimizer = torch.optim.Adam(
            model.parameters(), opt.learning_rate, weight_decay=opt.weight_decay)
    else:
    ...
    if opt.use_lr_scheduler:
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, step_size=opt.step_lr, gamma=0.1, last_epoch=-1)
    ...
    try:
        for epoch in range(0, opt.epochs):
            ...
            for i, data in enumerate(train_dataloader):
                ...
                if opt.do_validation and iteration_number % opt.evaluation_frequency == 0:
                    ...
                    num_eval_batches = math.ceil(num_samples / opt.batch_size)

                    print()
                    print("Train evaluation")
                    train_losses, train_metrics = evaluate.evaluate(
                        model, criterion, train_dataloader, num_eval_batches, "train")

                    print()
                    print("Val   evaluation")
                    val_losses, val_metrics = evaluate.evaluate(
                        model, criterion, val_dataloader, num_eval_batches, "val")

                    train_writer.add_scalar(
                        'Loss/Loss',        train_losses["total"],  iteration_number)
                    train_writer.add_scalar(
                        'Loss/Flow',        train_losses["flow"],   iteration_number)
                    train_writer.add_scalar(
                        'Loss/Graph',       train_losses["graph"], iteration_number)
                    train_writer.add_scalar(
                        'Loss/Warp',        train_losses["warp"],   iteration_number)
                    train_writer.add_scalar(
                        'Loss/Mask',        train_losses["mask"],   iteration_number)

                    train_writer.add_scalar(
                        'Metrics/EPE_2D_0',             train_metrics["epe2d_0"],      iteration_number)
                    train_writer.add_scalar(
                        'Metrics/EPE_2D_2',             train_metrics["epe2d_2"],      iteration_number)
                    train_writer.add_scalar(
                        'Metrics/Graph_Error_3D',       train_metrics["epe3d"],        iteration_number)
                    train_writer.add_scalar(
                        'Metrics/EPE_3D',               train_metrics["epe_warp"],     iteration_number)
                    train_writer.add_scalar(
                        'Metrics/ValidRatio',           train_metrics["valid_ratio"],  iteration_number)

                    val_writer.add_scalar(
                        'Loss/Loss',      val_losses["total"],    iteration_number)
                    val_writer.add_scalar(
                        'Loss/Flow',      val_losses["flow"],     iteration_number)
                    val_writer.add_scalar(
                        'Loss/Graph',     val_losses["graph"],   iteration_number)
                    val_writer.add_scalar(
                        'Loss/Warp',      val_losses["warp"],     iteration_number)
                    val_writer.add_scalar(
                        'Loss/Mask',      val_losses["mask"],     iteration_number)

                    val_writer.add_scalar(
                        'Metrics/EPE_2D_0',             val_metrics["epe2d_0"],      iteration_number)
                    val_writer.add_scalar(
                        'Metrics/EPE_2D_2',             val_metrics["epe2d_2"],      iteration_number)
                    val_writer.add_scalar(
                        'Metrics/Graph_Error_3D',       val_metrics["epe3d"],        iteration_number)
                    val_writer.add_scalar(
                        'Metrics/EPE_3D',               val_metrics["epe_warp"],     iteration_number)
                    val_writer.add_scalar(
                        'Metrics/ValidRatio',           val_metrics["valid_ratio"],  iteration_number)

                    print()
                    print()
                    print("Epoch number {0}, Iteration number {1}".format(
                        epoch, iteration_number))
                    print("{:<40} {}".format(
                        "Current Train Loss TOTAL",    train_losses["total"]))
                    print("{:<40} {}".format(
                        "Current Train Loss FLOW",     train_losses["flow"]))
                    print("{:<40} {}".format(
                        "Current Train Loss GRAPH",    train_losses["graph"]))
                    print("{:<40} {}".format(
                        "Current Train Loss WARP",     train_losses["warp"]))
                    print("{:<40} {}".format(
                        "Current Train Loss MASK",     train_losses["mask"]))
                    print()
                    print("{:<40} {}".format("Current Train EPE 2D_0",
                                             train_metrics["epe2d_0"]))
                    print("{:<40} {}".format("Current Train EPE 2D_2",
                                             train_metrics["epe2d_2"]))
                    print("{:<40} {}".format("Current Train EPE 3D",
                                             train_metrics["epe3d"]))
                    print("{:<40} {}".format("Current Train EPE Warp",
                                             train_metrics["epe_warp"]))
                    print("{:<40} {}".format(
                        "Current Train Solver Success Rate", train_metrics["valid_ratio"]))
                    print()

                    print("{:<40} {}".format(
                        "Current Val Loss TOTAL",      val_losses["total"]))
                    print("{:<40} {}".format(
                        "Current Val Loss FLOW",       val_losses["flow"]))
                    print("{:<40} {}".format(
                        "Current Val Loss GRAPH",      val_losses["graph"]))
                    print("{:<40} {}".format(
                        "Current Val Loss WARP",       val_losses["warp"]))
                    print("{:<40} {}".format(
                        "Current Val Loss MASK",       val_losses["mask"]))
                    print()
                    print("{:<40} {}".format(
                        "Current Val EPE 2D_0",            val_metrics["epe2d_0"]))
                    print("{:<40} {}".format(
                        "Current Val EPE 2D_2",            val_metrics["epe2d_2"]))
                    print("{:<40} {}".format(
                        "Current Val EPE 3D",              val_metrics["epe3d"]))
                    print("{:<40} {}".format("Current Val EPE Warp",
                                             val_metrics["epe_warp"]))
                    print("{:<40} {}".format(
                        "Current Val Solver Success Rate", val_metrics["valid_ratio"]))
                    ...
                    time_statistics.io_duration += (timer() - complete_cycle_start -
                                                    time_statistics.train_duration - time_statistics.eval_duration)
                ...
                time_statistics.forward_duration += (
                    timer() - train_batch_forward_pass)
                ...
                time_statistics.loss_eval_duration += (
                    timer() - train_batch_loss_eval)
                ...
                time_statistics.backward_duration += (
                    timer() - train_batch_backprop)
        ...
        snapshot_manager.save_model(
            model, iteration_number, final_iteration=True)
        raise err
